...
public class pccams : WebCamera
{
    ...
    [SerializeField][Tooltip("When running on PC (Editor/Standalone), pick this webcam index if available")] private int preferredDeviceIndex = 0;
    ...
    // Ensure PC webcam is initialized when in Editor/Standalone
    protected override void Awake()
    {
#if UNITY_EDITOR || UNITY_STANDALONE
        // On desktop we explicitly request webcam permission and select a device
        StartCoroutine(InitPcWebcam());
#else
        base.Awake();
#endif
    }

    private IEnumerator InitPcWebcam()
    {
        ...
        {
            Debug.LogWarning("No webcam devices found on this PC.");
            yield break;
        }

        // Clamp index and set device name to start the stream via base class
        int idx = Mathf.Clamp(preferredDeviceIndex, 0, devices.Length - 1);
